...
import (
	...
	"github.com/clubcedille/calidum-rotae-backend/cmd/cluster-provider/config"
	"github.com/clubcedille/calidum-rotae-backend/cmd/cluster-provider/server"
...
var (
	clusterCmd = &cobra.Command{
		Use:          "cluster_provider",
		Short:        "Cluster provider microservice for the calidum rotae app",
		SilenceUsage: true,
		RunE:         runClusterProvider,
	}
)

func init() {
	clusterCmd.Flags().Uint32(config.FlagPort, 0, "The gRPC port on which to listen to")
	clusterCmd.Flags().String(config.FlagLogLevel, "info", "The level of logs to print to stdout")

	clusterCmd.MarkFlagRequired(config.FlagPort)
}

func runClusterProvider(cmd *cobra.Command, args []string) (err error) {
	v := viper.New()
...
func main() {
	if err := clusterCmd.Execute(); err != nil {
		log.Fatalf("error when running the cluster provider: %s\n", err)
		os.Exit(1)
	}
}
