...
var GorpMigrationWhere = struct {
	...
	ID:        whereHelperstring{field: "\"calidum_rotae\".\"gorp_migrations\".\"id\""},
	AppliedAt: whereHelpernull_Time{field: "\"calidum_rotae\".\"gorp_migrations\".\"applied_at\""},
}
...
func GorpMigrations(mods ...qm.QueryMod) gorpMigrationQuery {
	mods = append(mods, qm.From("\"calidum_rotae\".\"gorp_migrations\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"calidum_rotae\".\"gorp_migrations\".*"})
	}
...
func FindGorpMigration(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*GorpMigration, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"calidum_rotae\".\"gorp_migrations\" where \"id\"=$1", sel,
	)
...
func (o *GorpMigration) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"calidum_rotae\".\"gorp_migrations\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"calidum_rotae\".\"gorp_migrations\" %sDEFAULT VALUES%s"
		}
...
func (o *GorpMigration) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"calidum_rotae\".\"gorp_migrations\" SET %s WHERE %s",
...
func (o GorpMigrationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"calidum_rotae\".\"gorp_migrations\" SET %s WHERE %s",
...
func (o *GorpMigration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"calidum_rotae\".\"gorp_migrations\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *GorpMigration) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"calidum_rotae\".\"gorp_migrations\" WHERE \"id\"=$1"
...
func (o GorpMigrationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"calidum_rotae\".\"gorp_migrations\" WHERE " +
...
func (o *GorpMigrationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"calidum_rotae\".\"gorp_migrations\".* FROM \"calidum_rotae\".\"gorp_migrations\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, gorpMigrationPrimaryKeyColumns, len(*o))
...
func GorpMigrationExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"calidum_rotae\".\"gorp_migrations\" where \"id\"=$1 limit 1)"
